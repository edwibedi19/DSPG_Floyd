...
floyd %>%
  ...
  geom_point(data=locations_features_floyd, aes(x=Longitude1, y=Latitude), color = "gold", alpha = .2, size = 5) + 
...
scale_y_continuous("Percent", sec.axis = sec_axis(~.*1000, name = "Depth (ft)"))  + 
  ggtitle("Percent of Usage and Well Depth of Wells in Floyd County")
...
gc <- ggplot(climate, aes(x = Month, y = Rainfall)) +
  ...
  labs(x = "Month", 
       ...
       subtitle = "*This data is the average of each month from 1980-2010")
...
max_temp <- ggplot(climate, aes(fill = County, x = Month, y = Max_Temp)) + 
  geom_bar(position="dodge", stat="identity") + 
  scale_x_discrete(limits = month.abb) + 
  labs(title = "Max Tempature from 3 surrounding counties in Virginia",
       caption = "Data Source: US Climate Data",
...
min_temp <- ggplot(climate, aes(fill = County, x = Month, y = Min_Temp)) + 
  geom_bar(position="dodge", stat="identity") + 
  scale_x_discrete(limits = month.abb) +
   labs(title = "Min Tempature from 3 surrounding counties in Virginia",
       caption = "Data Source: US Climate Data")
